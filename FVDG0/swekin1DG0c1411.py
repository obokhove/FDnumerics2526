...
if nRP==0:
 ...
 FA00 = fd.Function(DG0).interpolate( fd.conditional( A0<wb*hb, sqrtmslope*A0**(5/3)/(wb+2.0*A0/wb)**(2/3)/Cm , \
                                                      sqrtmslope*A0**(5/3)/(wb+2*hb+widL-wb+2.0*(A0-hb*wb)/widL)**(2/3)/Cm ) ) # TC2
...
if nRP==0:
 #
 widL0 = width2ufl(w0, w1, w2, xa, xb, k1, k2, xc, xd, 0.0) # UPDATE: better to use width at x=0 (s=0)
