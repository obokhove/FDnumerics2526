...
if Nbc == 4:
    ...
    tmease = 0.0
    dtmeas = Tend / nmea
    Qmax = 1 * 350  # UPDATE TC0, TC2-pretest) 0 constant influx; TC1/TC2) nonzero varying influx; on top of base flux
    tmax = 0.5 * Tend
    gamfac = 0.000001
...
if Nbc == 4:  # flow
    ...
    if nRP == 0:
        ...
        w00 = fd.Function(DG0).interpolate(wbb + 0.0 * x)
        h00 = fd.Function(DG0).interpolate(H0 + 0.0 * x)
...
widvals = 0.0 * xvals
fig, (ax1, axL, ax2, ax3) = plt.subplots(4)
tsize = 14
...
ax1.grid()
axL.set_ylabel(r'$w(s,t)$ (m)', fontsize=tsize)
...
if nRP == 0:
    ...
    FA00 = fd.Function(DG0).interpolate(
        sqrtmslope * A0 ** (5 / 3) / (widL + 2.0 * A0 / widL) ** (2 / 3) / Cm)  # TC0/TC1
    h00 = fd.Function(DG0).interpolate(A0 / widL)
elif nRP == 1:
    phi12 = np.array([wid0.at(x) for x in xvals])
    w00 = fd.Function(DG0).interpolate(fd.conditional(A0 < Ab, wbb, sqrt(wbb**2 +400*(A0-Ab))))
    w00_c = fd.Function(CG1).interpolate(fd.conditional(A0 < Ab, wbb, sqrt(wbb**2 +400*(A0-Ab)))) # in code but not actually used now
    h00 = fd.Function(DG0).interpolate(fd.conditional(A0 < Ab, A0 / wbb, hbb + 0.01*(w00-wbb)/2))
    FA00 = fd.Function(DG0).interpolate(
        fd.conditional(A0 < Ab, sqrtmslope * A0 ** (5 / 3) / (wbb + 2.0 * A0 / wbb) ** (2 / 3) / Cm,
                       sqrtmslope * A0 ** (5 / 3) / (wbb + 2 * hbb  + (4*sqrt(1+100**2)*(A0-Ab)/(w00+wbb))) ** (
                                   2 / 3) / Cm))  # TC2
Q12 = np.array([FA00.at(x) for x in xvals])  #
eta12 = np.array([h00.at(x) for x in xvals])  #
area = np.array([A0.at(x) for x in xvals])
width = np.array([w00.at(x) for x in xvals])
ax1.plot(xvals, eta12)
axL.plot(xvals, width)
ax2.plot(xvals, area) #change A, as this is not right (use A0 as this is wL*h, ahould be wb*hb+wL*(h-hb))
ax3.plot(xvals, Q12)
...
if nRP == 0:
    ...
    #
    A0left = fd.Constant(
        H0 * wb)  # constant inflow for test case UPDATE to conditional works only if H0<hb; now cannot work when Q0 large but fixed below in time loop
    A0left0 = H0 * wb
    #
...
if nRP == 0:
    ...
    #
    FA0 = fd.conditional(A0 < Ab, sqrtmslope * A0 ** (5 / 3) / (wbb + 2.0 * A0 / wbb) ** (2 / 3) / Cm,
                       sqrtmslope * A0 ** (5 / 3) / (wbb + 2 * hbb  + (4*sqrt(1+100**2)*(A0-Ab)/(w00+wbb))) ** (
                                   2 / 3) / Cm)
    #w0 = fd.conditional(A0 < Ab, wbb, sqrt(wbb**2 +400*(A0-Ab)))
    if A0left0 < hb * wb:
        FA0left0 = sqrtmslope * A0left0 ** (5 / 3) / (wb + 2.0 * A0left0 / wb) ** (
                2 / 3) / Cm  # FA0left0 does not need fd.Constant
        w0 = wb
    else:
        w0 = np.sqrt(wb**2 + 400*(A0left0 - hb*wb))
        FA0left0 = sqrtmslope * A0left0 ** (5 / 3) / (wb + 2 * hb  + (4*sqrt(1+100**2)*(A0left0-hb*wb)/(w0+wb))) ** (
                2 / 3) / Cm  # FA0left0 does not need fd.Constant
    FA0left.assign(FA0left0)
    print('A0left0,FA0left0', A0left0, FA0left0)
    Peromi = fd.conditional(A0('-') < Ab, wbb + 2 * A0('-') / wbb,
                            wbb + 2 * hbb + 4 * sqrt(1+100**2)*(A0('-')-Ab)/(w00('-')+wbb))
    ...
    dFA0dA0 = (sqrtmslope / (3 * Cm)) * A0avg ** (2 / 3) * fd.conditional(A0avg < Ab, (5 * wbb + 6 * A0avg / wbb) / (
            wbb + 2.0 * A0avg / wbb) ** (5 / 3), \
                                                                          (5 * (
                                                                                  wbb + 2 * hbb + (4*sqrt(1+100**2)*(A0avg-Ab)/(w00avg+wbb))) - (8*sqrt(1+100**2)*A0avg/ (w00avg+wbb))) / (
                                                                                  wbb + 2 * hbb + (4*sqrt(1+100**2)*(A0avg-Ab)/(w00avg+wbb))) ** (
                                                                                  5 / 3))  #
    dFA0dA0bnd = (sqrtmslope / (3 * Cm)) * A0 ** (2 / 3) * fd.conditional(A0 < Ab, (5 * wbb + 6 * A0 / wbb) / (
            wbb + 2.0 * A0 / wbb) ** (5 / 3), \
                                                                          (5 * (
                                                                                  wbb + 2 * hbb + (4 * sqrt(
                                                                              1 + 100 ** 2) * (A0-Ab) / (
...
while t <= Tend:  # while nt<10:
    # assigning w0 and h0 at beginning of each time step
    # issues if not
    # I think  because w0 depends on A0, but then you recalculate A0 (which depends on w0)
    # so this makes sure w0 is calculated using A_n not A_n+1
    w00.assign(fd.Function(DG0).interpolate(fd.conditional(A0 < Ab, wbb, sqrt(wbb ** 2 + 400 * (A0 - Ab)))))
    w00_c.assign(fd.Function(CG1).interpolate(fd.conditional(A0 < Ab, wbb, sqrt(wbb ** 2 + 400 * (A0 - Ab)))))
    h00.assign(fd.Function(DG0).interpolate(fd.conditional(A0 < Ab, A0 / wbb, hbb + 0.01 * (w00 - wbb) / 2)))
    t += dt
    nt = nt + 1
    ...
    A0.assign(A1)
    #w00 = fd.Function(DG0).interpolate(fd.conditional(A0 < Ab, wbb, sqrt(wbb ** 2 + 400 * (A0 - Ab))))
    #w00_c = fd.Function(CG1).interpolate(fd.conditional(A0 < Ab, wbb, sqrt(wbb ** 2 + 400 * (A0 - Ab))))
    #h00 = fd.Function(DG0).interpolate(fd.conditional(A0 < Ab, A0 / wbb, hbb + 0.01 * (w00 - wbb) / 2))
    #
    #
    #
    ...
    if t > tmease + smallfac:  # if nt>0:
        ...
        Q12 = np.array([FA00.at(x) for x in xvals])
        width = np.array([w00.at(x) for x in xvals])
        label_str = f"t = {t / 3600:.2f} h"
        ax1.plot(xvals, eta12,label=label_str)
        axL.plot(xvals, width)
        ax2.plot(xvals, area)
        ax3.plot(xvals, Q12)  #
    # if t>tmE+smallfac:
...
axL.plot(xvals, width, '--k', linewidth=3)
...
df = pd.DataFrame({"height": eta12, "Q": Q12, "A": phi12*eta12, "x": xvals})
df.to_csv(f"{Nx}_{10*CFL}_t3.csv")
plt.show()
