...
if Nbc==4:
    ...
    CFL = 0.125*1.0 #0.125,0.25,0.5,1
    Lx = 5000 #length of river channel considered
    grav = 9.81
    xo = 1000
    xa = 1500-xo
    xb = 1600-xo
    ...
    wb = 20
    hb = 4
    nRP = 0 # 0 for TC0 and TC2b, 1 for TC1 and TC2b ;
    ...
    Cm = 0.1
    Nx = 10000 #use this to define space step - 10000 (0.5 dx), 5000, 2500, 1250
    dxx = Lx/Nx
    c00 = np.sqrt(grav*H0)
    ...
    tmease = 0.0
    dtmeas = Tend/nmea
    Qmax = 1*350  # TC0, TC2-pretest) 0 constant influx; TC1/TC2) nonzero varying influx; on top of base flux
    tmax = 0.5*Tend
    gamfac = 0.000001
...
while t <= Tend: #while nt<10:
    ...
    t_.assign(t)
    # Variable time step:
    # dFdmax = fd.maximum( (1/3)*sqrtmslope*A0**(2/3)*( 5*widL+6*A0avg/widL )/(widL+2.0*A0/widL)**(5/3)/Cm ) #F_A calculation - shock speed
    # dt = CFL*dxx/dFmax
    #
    ...
    if t>tmease+smallfac: # if nt>0:
        ...
        if nRP==0:
          ...
          h00 = fd.Function(DG0).interpolate( fd.conditional( A0<Ab, A0/wbb, hbb+(A0-Ab)/widL) )
        eta12 = np.array([h00.at(x) for x in xvals]) # height
        area = np.array([A0.at(x) for x in xvals]) #width
        Q12 = np.array([FA00.at(x) for x in xvals]) #flux
        label_t = f"t = {t / 3600 : .2f}"
        ax1.plot(xvals,eta12, label = label_t)
        ax2.plot(xvals,area)
        ax3.plot(xvals,Q12) #
        ax1.legend(title = 'time (hrs)', bbox_to_anchor = (1.5,0))
    #if t>tmE+smallfac:
    plt.figure(2)
    plt.plot(t,Q00,'.')
...
plt.show()
df = pd.DataFrame({"h(s,t)": eta12, "A(s,t)": area, "Q(s,t)": Q12, "s": xvals})
df.to_csv(f"TC1_fine_Nx={Nx}_CFL={CFL}.csv")
